...
var deanCollection *mongo.Collection = configs.GetCollection(configs.DB, "dean_user")
var validate_user = validator.New()

// Find User By Email Search
func FindDeanBy_UniversityId(ctx context.Context, universityid *string) (models.User, error) {
	var foundDean models.User
	err := deanCollection.FindOne(ctx, bson.M{"universityid": universityid}).Decode(&foundDean)
	if err != nil {
		return foundDean, err
	}
	return foundDean, err
}

// HashPassword is used to encrypt the password before it is stored in the DB
func HashPassword_Dean(password string) string {
...
func VerifyPassword_Dean(userPassword string, providedPassword string) (bool, string) {
...
func Register_Dean(c *fiber.Ctx) error {
	...
	err := deanCollection.FindOne(ctx, bson.M{"universityid": user.UniversityId}).Decode(&user)

	if err == nil {
		return c.Status(http.StatusConflict).JSON(fiber.Map{"message": "Dean already exists with this Unversity Id"})
	}
	...
	result, err := deanCollection.InsertOne(ctx, user)
...
func Login_Dean(c *fiber.Ctx) error {
	...
	foundDean, err := FindDeanBy_UniversityId(ctx, &user.UniversityId)
	if err != nil {
...
func Dean(c *fiber.Ctx) error {
	...
	err = deanCollection.FindOne(ctx, bson.M{"id": _id}).Decode(&foundDean)
...
func Logout_Dean(c *fiber.Ctx) error {
	cookie := fiber.Cookie{
		Name:     "jwt",
		Value:    "",
